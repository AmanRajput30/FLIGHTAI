...
If Averyn creates a Derivative Database, ODbL may require:
...
Averyn operates exclusively on short-lived caching for ADSB.lol data to reduce upstream request load. **NO long-term historical ADSB.lol data is persisted.**

## Separation of Proprietary Data
Ensure that ADSB.lol data is never permanently mixed into Averyn's proprietary databases (e.g., users, billing, saved aircraft, SkyLord configuration) to preserve clear licensing boundaries. 
