...
export const flightApi = {
  getSearch: async (query: string): Promise<any> => {
  ...
  getFlightPath: async (flightId: string): Promise<[number, number][]> => {
    ...
    if (response.data && response.data.path) {
      return response.data.path.map((pt: any) => [pt[1], pt[2]]);
    }
    return [];
  },
};
...
export const chatApi = {
  sendMessage: async (messages: ChatMessage[], flightContext: Flight | null): Promise<any> => {
