    ...
    <div className="flex w-full min-h-screen bg-aervyn-bg-dark">
      ...
      >
        ...
        <div className="absolute top-8 right-8 hidden lg:flex items-center gap-4">
          <span className="text-sm text-aervyn-text-dark-secondary">New to AERVYN?</span>
          <Link href="/register" className="text-sm font-medium text-aervyn-primary bg-aervyn-primary/10 hover:bg-aervyn-primary/20 px-4 py-2 rounded-full transition-colors border border-aervyn-primary/20">
            Create account
          </Link>
        </div>
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-5">
          <div>
            <label className="block text-sm font-medium text-aervyn-text-dark-primary mb-1.5">Username or email</label>
            <input
              type="text"
              required
              value={identifier}
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label className="block text-sm font-medium text-aervyn-text-dark-primary">Password</label>
            </div>
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-4 top-1/2 -translate-y-1/2 text-aervyn-text-dark-muted hover:text-aervyn-text-dark-primary transition-colors"
              >
          ...
          <div className="flex items-center justify-between mt-1">
            <label className="flex items-center gap-2 cursor-pointer">
              <input 
                ...
                className="w-4 h-4 rounded border-aervyn-border-dark bg-aervyn-bg-dark text-aervyn-primary focus:ring-aervyn-primary" 
              />
              <span className="text-sm text-aervyn-text-dark-secondary">Keep me signed in</span>
            </label>
            <Link href="/forgot-password" className="text-sm font-medium text-aervyn-primary hover:text-aervyn-primary-hover transition-colors">
              Forgot password?
            </Link>
          </div>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full mt-2 bg-aervyn-primary hover:bg-aervyn-primary-hover text-white py-2.5 rounded-lg font-medium text-sm transition-colors duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed border border-aervyn-primary"
          >
            {isLoading ? (
        ...
        <div className="mt-8 flex justify-center">
          <div className="flex items-center gap-2">
            <span className="text-sm text-aervyn-text-dark-secondary">New to AERVYN?</span>
            <Link href="/register" className="text-sm font-medium text-aervyn-primary hover:text-aervyn-primary-hover transition-colors">
              Create an account
            </Link>
          </div>
        </div>
        ...
        <div className="relative my-8">
          ...
          <div className="relative flex justify-center">
            <span className="bg-aervyn-surface-dark px-4 text-xs text-aervyn-text-dark-muted uppercase tracking-wider font-medium">Or continue with</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-4">
          <button 
            type="button" 
            onClick={() => googleLogin()}
            className="w-full bg-aervyn-bg-dark border border-aervyn-border-dark hover:border-aervyn-border-dark-subtle text-aervyn-text-dark-primary py-2.5 rounded-lg font-medium text-xs sm:text-sm transition-colors flex items-center justify-center gap-2"
          >
        ...
        <div className="absolute bottom-8 right-8 flex gap-4 text-xs text-aervyn-text-dark-muted">
          <Link href="/privacy" className="hover:text-aervyn-text-dark-primary transition-colors">Privacy</Link>
          <Link href="/terms" className="hover:text-aervyn-text-dark-primary transition-colors">Terms</Link>
          <Link href="/contact" className="hover:text-aervyn-text-dark-primary transition-colors">Support</Link>
        </div>
        </div>
      </motion.div>
    </div>
